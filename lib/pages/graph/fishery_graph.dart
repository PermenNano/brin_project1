...
class _FisheryGraphState extends State<FisheryGraph> {
  ...
  final List<String> _fisherySensorIds = [
    'DO001', 'HUM01', 'TEM01', 'RSS01', 'TDS01'
  ];
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (dataList != null && dataList is List) {
          ...
          _availableSensorsFiltered = _availableSensorsForFarm.where((id) => _fisherySensorIds.contains(id)).toList();
          ...
          if (_selectedSensorId != null) {
            ...
            setState(() {
              isLoading = false;
              errorMessage = 'No sensors available for this farm';
              _showGraph = false;
            });
          }
        } else {
  ...
  Widget _buildNoDataOrInitialScreen() {
    ...
    if (_availableSensorsFiltered.isEmpty && !isLoading) {
      return Center(
        child: Text(
          'No sensors available for this farm',
